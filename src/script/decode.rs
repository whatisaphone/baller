...
fn decode_ins<'a>(code: &mut &'a [u8]) -> Option<Ins<'a>> {
    ...
    match opcode {
        ...
        0x6d => ins!([0x6d], name = "get-class", args = [int, list], retval),
        ...
        0x8d => ins!([0x8d], name = "get-x", args = [int], retval),
        0x8e => ins!([0x8e], name = "get-y", args = [int], retval),
...
fn op_26_sprite<'a>(code: &mut &'a [u8]) -> Option<Ins<'a>> {
    match read_u8(code)? {
        ...
        0x3f => ins!([0x26, 0x3f], name = "sprite-set-image", args = [int]),
...
fn op_6b_cursor<'a>(code: &mut &'a [u8]) -> Option<Ins<'a>> {
    match read_u8(code)? {
        ...
        0x90 => ins!([0x6b, 0x90], name = "cursor-show"),
        0x91 => ins!([0x6b, 0x91], name = "cursor-hide"),
        0x92 => ins!([0x6b, 0x92], name = "cursor-input-enable"),
        0x93 => ins!([0x6b, 0x93], name = "cursor-input-disable"),
        0x94 => ins!([0x6b, 0x94], name = "cursor-visible-increment"),
        0x95 => ins!([0x6b, 0x95], name = "cursor-visible-decrement"),
